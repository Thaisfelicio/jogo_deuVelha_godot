...
@export var cena_pilula: PackedScene
		...
		if event.button_index == MOUSE_BUTTON_LEFT and event.pressed:
			if event.position.x < tamanho_tabuleiro:
				...
				if dados_grade[posicao_grade.y][posicao_grade.x] == 0:
					...
					if posicao_grade == posicao_pilula:
						var pilula = cena_pilula.instantiate()
						pilula.position = posicao_grade * tamanho_celula + Vector2i(tamanho_celula / 2, tamanho_celula / 2)
						add_child(pilula)
						move_child(pilula, 2)
						print("Você achou uma pílula")
						$Tela_escolha.show()
...
func novo_jogo():
	...
	#colocar a pilula em uma celula aleatoria
	posicao_pilula = Vector2i(randf() * 3, randf() * 3)
	print("Posição da pílula: ", posicao_pilula)
...
func _on_tela_escolha_escolha_apagar():
	if movimentos > 0:
		# Encontrar e remover a última jogada
		var ultima_jogada_pos = Vector2i(-1, -1)
		for y in range(2, -1, -1):
			for x in range(2, -1, -1):
				if dados_grade[y][x] != 0:
					dados_grade[y][x] = 0
					movimentos -= 1
					# Remover o marcador visual
					for child in get_children():
						if child.position == Vector2i(x, y) * tamanho_celula + Vector2i(tamanho_celula / 2, tamanho_celula / 2):
							child.queue_free()
							break
					break
			if movimentos < 1:
...
func _on_tela_escolha_escolha_bloquear():
	jogador *= -1
	$Tela_escolha.hide()
